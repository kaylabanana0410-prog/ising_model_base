...
FC1_PATH = DATA_ROOT / "FC data_processed" / "avg_TS_1p"
FC2_PATH = DATA_ROOT / "FC data_processed" / "avg_TS_2p"
FC3_PATH = DATA_ROOT / "FC data_processed" / "avg_TS_3p"
...
ANNEAL_MAXFUN = 50 #500
ANNEAL_THERM  = 500#5000
SWEEP_STEPS   = 1000
SWEEP_THERM   = 2000


N_RESTARTS    = 1 ##  20 was 5 :5 annealing runs and picks the best one
...
T_MIN         = 2 #### IT WAS 1 BEFORE WITH RESULTS IN GOOGLWE DOC 
T_MAX         = 20 # 10
T_STEPS       =200#400 was 100
TEMP_REPEATS  = 5
...
def safe_pearson(x, y):
   """matrix r that returns 0 instead of NaN for constant or non-finite vectors."""
   x = clean_vec(x)
   y = clean_vec(y)
...
# ── data ──────────────────────────────────────────────────────────────────
# Partial FC analysis uses the original structural Jij directly.
# The thresholded Pearson-signed Jij is only used by pearson_2.py.
J_real  = avg_Jij.copy()
...
rho_emp = (emp_FC1 + emp_FC2 + emp_FC3) / 3.0          # partial empirical FC throughout
# Use non-negative coupling strength for temperature scaling.
# Signed Jij can otherwise produce negative multipliers, and
...
plt.savefig(RESULTS_DIR / "param_anneal_error_4.png", dpi=150, bbox_inches="tight")
plt.close()
...
T_crit    = sweep.T_global[np.nanargmax(suscept_arr)]
...
sweep.crit_ising = sweep.ising_ar[np.nanargmax(suscept_arr)]


T_global = sweep.T_global


print(f"\nCritical temperature (peak susceptibility) : {T_crit:.4f}")
print(f"Best-match temperature (peak r)        : {T_best:.4f}")
print(f"Best matrix r                         : {best_corr:.4f}")
...
for ax, data, sd, ylabel, title in panels:
   ...
   ax.fill_between(T_global, data - sd, data + sd, color=BLUE, alpha=0.18, linewidth=0)
   ...
   ax.axvline(
       ...
       linestyle="--",
       lw=1.8,
       label=f"$T_{{crit}}$ = {T_crit:.2f}  (peak χ)"
   )
...
plt.savefig(RESULTS_DIR / "temperature_sweep_4.png", dpi=150, bbox_inches="tight")
plt.close(fig1)
...
for data, sd, color, label, lw, alpha in [
   (np.array(sweep.corr_ar_total), np.array(sweep.corr_sd_ar_total), BLUE, "avg FC", 2, 1.0),
   (np.array(sweep.corr_ar_1), np.array(sweep.corr_sd_ar_1), "green", "FC1", 1, 0.7),
   (np.array(sweep.corr_ar_2), np.array(sweep.corr_sd_ar_2), "purple", "FC2", 1, 0.7),
   (np.array(sweep.corr_ar_3), np.array(sweep.corr_sd_ar_3), "orange", "FC3", 1, 0.7),
]:
   ax_corr.plot(T_global, data, color=color, lw=lw, label=label, alpha=alpha)
   ax_corr.fill_between(T_global, data - sd, data + sd, color=color, alpha=0.12, linewidth=0)
...
plt.savefig(RESULTS_DIR / "correlation_vs_T_4.png", dpi=150, bbox_inches="tight")
plt.close(fig_corr)
...
plt.savefig(RESULTS_DIR / "matrix_comparison_4.png", dpi=150, bbox_inches="tight")
plt.close(fig3)
...
plt.savefig(RESULTS_DIR / "scatter_sim_vs_emp_4.png", dpi=150, bbox_inches="tight")
plt.close(fig3s)
...
if len(post_crit_indices) > 0:
   ...
   plt.savefig(RESULTS_DIR / "matrix_comparisons_post_Tcrit_4.png", dpi=150, bbox_inches="tight")
   plt.close(fig3_post)
...
def run_ising_avg(J, T_global_value, alpha, n_runs=NULL_RUNS):
   """
   Run null Ising model and return a finite partial FC matrix.
...
plt.savefig(RESULTS_DIR / "ising_null_distributions_4.png", dpi=150, bbox_inches="tight")
plt.close(fig4)
...
plt.savefig(RESULTS_DIR / "ising_null_distributions_ones_4.png", dpi=150, bbox_inches="tight")
plt.close(fig4_ones)
...
print(f"T_crit            = {T_crit:.4f}  (peak susceptibility)")

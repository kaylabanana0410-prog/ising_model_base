...
def monte_carlo_metropolis(J, spin_vec, t, iterations, thermalize_time=None, phi_variables=False, type='digital', alpha=0, return_tc=False):
    ...
    for i in range(int(iterations / no_spin)):
        ...
        for j in range(no_spin):
            ...
            #print(t[j]*mu[j]**alpha)
            if d_e <= 0 or random() <= np.exp(-d_e / t*mu[j]**alpha): # -d_e * T_local <- array of T * mu ^ alpha
    ...
    if thermalize_time is not None:
        ...
        if phi_variables:
            ...
            M = np.flipud(np.fliplr(np.asarray(M).astype(int)))
            M = M * 2 - 1
...
def compute_par(values):
    ...
    for tT in range(values[1], values[2]):
        ...
        if phi_variables:
            es, ess, ms, mss, spin_bin_sum = monte_carlo_metropolis(values[0], spin_vec, ts[tT], no_flip, values[4], phi_variables=phi_variables, type=type, alpha=values[9])
            spin_mean.append(spin_bin_sum / avg_therm)
        else:
            es, ess, ms, mss = monte_carlo_metropolis(values[0], spin_vec, ts[tT], no_flip, values[4], phi_variables=phi_variables, type=type, alpha=values[9])
        ...
        for sim in range(values[3]):
            simulation[:, sim] = monte_carlo_metropolis(values[0], spin_vec, ts[tT], n, phi_variables=phi_variables, type=type, alpha=values[9])
...
def generalized_ising(Jij, temperature_parameters=(0.1, 5, 100), n_time_points=100, thermalize_time=0.3, temperature_distribution ='lineal', phi_variables = False, return_tc = False, type='digital', alpha=0):
    n = Jij.shape[-1]
    ...
    for next in range(n_cpu):
        if (next + 1) * step_len > temperature_parameters[2]:
            l.append((Jij, previus, temperature_parameters[2], n_time_points, thermalize_time, ts, phi_variables, type, return_tc, alpha))
        else:
            l.append((Jij, previus, int((next + 1) * step_len), n_time_points, thermalize_time, ts, phi_variables, type, return_tc, alpha))

        previus = int((next + 1) * step_len)

    results = pool.map(compute_par, l)
    print(len(results))
